"""Third comb-density SOC frequency-enrichment run.

E4f is the strict E2f refinement: base K=400 plus a 0.03125 fill on
[4, 16].  Rows are solved at low-mean anchors, exact-budget hardened, then
inserted into the inherited dense SOC row envelope.
"""

from __future__ import annotations

import json
import time
from pathlib import Path
from typing import Any

import numpy as np

import sys

sys.path.insert(0, "source")
import soc_e1_certificate as e1c  # noqa: E402
import soc_freq_enrich2 as fe2  # noqa: E402
import soc_full_envelope as sfe  # noqa: E402
import soc_dual as sd  # noqa: E402
import soc_menvelope_harden as smh  # noqa: E402


ROOT = Path("source")
DATA = ROOT / "data"
BASE_CERT_PATH = ROOT / "soc_certificate_improved.json"
BASE_EXACT_BUDGET_PATH = DATA / "soc_budget_exact.json"
PRIOR_JSON = DATA / "soc_freq_enrich2.json"
OUT_JSON = DATA / "soc_freq_density3.json"
OUT_JSON_ROOT_COPY = ROOT / "soc_freq_density3.json"
ROW_CACHE_DIR = DATA / "soc_freq_density3_rows"

BASE_CERTIFIED_GLOBAL = 0.38043481690472875
E1_CERTIFIED_VALUE = 0.380496768638831
E2F_CERTIFIED_VALUE = 0.380530042996299
KNOWN_FEASIBLE_GUARD = 0.380895
ACTIVE_BRACKETS = [(0.0, 0.03), (0.0, 0.04)]
E4F_ANCHORS_REQUESTED = [0.0, 0.001, 0.002, 0.003, 0.004475, 0.006]
E4F_PRIORITY_SCHEDULE = [0.0, 0.003, 0.001, 0.002, 0.004475, 0.006]
ROOT_GRID_N = 1_000_000
ROOT_TOL = 5.0e-13
EXACT_DPS = 80
REFINED_PARTITIONS = 60_000
DOMAIN_PARTITIONS = 400_000
MEAN_GUARD_EXPECTED = 5.0e-12


def _jsonify(value: Any) -> Any:
    if isinstance(value, np.ndarray):
        return value.tolist()
    if isinstance(value, (np.floating, np.integer)):
        return value.item()
    if isinstance(value, dict):
        return {str(k): _jsonify(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_jsonify(v) for v in value]
    return value


def _load_json(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def _write_json(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(_jsonify(payload), indent=2, sort_keys=True) + "\n", encoding="utf-8")


def _fill_grid(lo: float, hi: float, step: float) -> np.ndarray:
    count = int(round((float(hi) - float(lo)) / float(step)))
    return np.asarray([float(lo) + float(step) * i for i in range(count + 1)], dtype=np.float64)


def _build_frequency_sets(base_xi: np.ndarray) -> dict[str, np.ndarray]:
    prior = fe2._build_frequency_sets(base_xi)
    e4f_xi = np.union1d(np.asarray(base_xi, dtype=np.float64), _fill_grid(4.0, 16.0, 0.03125))
    return {**prior, "e4f": e4f_xi}


def _inclusion_check(freqs: dict[str, np.ndarray]) -> dict[str, Any]:
    base = np.asarray(freqs["base"], dtype=np.float64)
    e1 = np.asarray(freqs["e1"], dtype=np.float64)
    e2f = np.asarray(freqs["e2f"], dtype=np.float64)
    e4f = np.asarray(freqs["e4f"], dtype=np.float64)
    missing_base = np.setdiff1d(base, e4f)
    missing_e1 = np.setdiff1d(e1, e4f)
    missing_e2f = np.setdiff1d(e2f, e4f)
    extra_vs_e2f = np.setdiff1d(e4f, e2f)
    return {
        "base_frequency_count": int(base.size),
        "e1_frequency_count": int(e1.size),
        "e2f_frequency_count": int(e2f.size),
        "e4f_frequency_count": int(e4f.size),
        "e4f_strict_superset_of_base": bool(e4f.size > base.size and missing_base.size == 0),
        "e4f_strict_superset_of_e1": bool(e4f.size > e1.size and missing_e1.size == 0),
        "e4f_strict_superset_of_e2f": bool(e4f.size > e2f.size and missing_e2f.size == 0),
        "missing_base_frequency_count": int(missing_base.size),
        "missing_e1_frequency_count": int(missing_e1.size),
        "missing_e2f_frequency_count": int(missing_e2f.size),
        "extra_vs_e2f_count": int(extra_vs_e2f.size),
        "all_requested_inclusions_pass": bool(
            e4f.size > e2f.size
            and missing_base.size == 0
            and missing_e1.size == 0
            and missing_e2f.size == 0
        ),
    }


def _comb_description() -> str:
    return "base K=400 plus 0.03125-grid fill on [4,16]"


def _cache_path(anchor_m: float) -> Path:
    safe = f"{float(anchor_m):.7f}".replace(".", "p")
    return ROW_CACHE_DIR / f"e4f_m_{safe}.json"


def _load_cached_e4f_row(anchor_m: float) -> dict[str, Any] | None:
    path = _cache_path(anchor_m)
    if not path.exists():
        return None
    payload = _load_json(path)
    row = payload.get("row")
    if row and row.get("exact_positive_part_budget", {}).get("exact_budget_pass") and row.get("real_generator_gate_pass"):
        payload = dict(payload)
        payload["cache_status"] = "loaded_from_cache"
        return payload
    return None


def _load_prior_enrichment_payloads() -> list[dict[str, Any]]:
    if not PRIOR_JSON.exists():
        return []
    prior = _load_json(PRIOR_JSON)
    payloads: list[dict[str, Any]] = []
    for summary in prior.get("completed_rows", []):
        path = Path(summary["cache_path"])
        if not path.exists() and not path.is_absolute():
            path = fe2.ROW_CACHE_DIR / path.name
        if not path.exists():
            continue
        payload = _load_json(path)
        row = payload.get("row")
        if not row:
            continue
        if not row.get("exact_positive_part_budget", {}).get("exact_budget_pass"):
            continue
        if not row.get("real_generator_gate_pass"):
            continue
        payload = dict(payload)
        payload["cache_status"] = "loaded_from_soc_freq_enrich2_cache"
        payloads.append(payload)
    return payloads


def _row_for_exact_budget(comp: dict[str, Any], anchor_m: float) -> dict[str, Any]:
    return {
        "id": f"e4f_anchor_m_{anchor_m:.7f}",
        "m": float(anchor_m),
        "K": int(comp["selected_frequency_count"]),
        "positive_part_partitions": 200_000,
        "alpha_boundary_threshold": 1.0e-10,
        "coefficient_intervals": {
            "a0_raw": sfe._interval(float(comp["a0"])),
            "a1": sfe._interval(float(comp["a1"])),
            "a2": sfe._interval(float(comp["a2"])),
        },
        "atom_intervals": {
            "xi": [sfe._interval(float(v)) for v in np.asarray(comp["xi"], dtype=np.float64)],
            "alpha": [sfe._interval(float(v)) for v in np.asarray(comp["alpha"], dtype=np.float64)],
            "beta": [sfe._interval(float(v)) for v in np.asarray(comp["beta"], dtype=np.float64)],
        },
    }


def _hardened_row(comp: dict[str, Any], budget: dict[str, Any], anchor_m: float) -> dict[str, Any]:
    alpha = np.asarray(comp["alpha"], dtype=np.float64)
    beta = np.asarray(comp["beta"], dtype=np.float64)
    return {
        "id": f"e4f_anchor_m_{anchor_m:.7f}",
        "family": "e4f",
        "m": float(anchor_m),
        "K": int(comp["selected_frequency_count"]),
        "selected_frequency_count": int(comp["selected_frequency_count"]),
        "nominal_raw_L": float(comp["raw_L"]),
        "quadratic_c0_lower": float(budget["quadratic_c0_lower"]),
        "L_source_m_lower": float(budget["L_source_m_lower"]),
        "a1": float(comp["a1"]),
        "a2": float(comp["a2"]),
        "exact_positive_part_budget": budget,
        "real_generator_gate_pass": bool(np.all(alpha[np.abs(beta) > 0.0] > 0.0)),
        "min_alpha_where_beta_nonzero": (
            float(np.min(alpha[np.abs(beta) > 0.0])) if np.any(np.abs(beta) > 0.0) else None
        ),
        "beta_active_count_gt_1e_8": int(np.sum(np.abs(beta) > 1.0e-8)),
        "source": {
            "description": "E4f fixed-mean SOC witness.",
            "comb_rule": _comb_description(),
        },
    }


def _solve_and_harden_e4f_row(xi: np.ndarray, anchor_m: float) -> dict[str, Any]:
    cached = _load_cached_e4f_row(anchor_m)
    if cached is not None:
        print(f"row_cache_hit family=e4f m={anchor_m:.7f}", flush=True)
        return cached

    print(f"row_solve_start family=e4f m={anchor_m:.7f}", flush=True)
    started = time.time()
    sol = sd.solve_fixed_mean_soc(
        np.asarray(xi, dtype=np.float64),
        m=float(anchor_m),
        x_grid_points=1001,
        solver="CLARABEL",
        max_iter=500,
        tol=1.0e-8,
    )
    comp = sd.compress_zero_pairs(sol)
    print(
        f"row_nominal_done family=e4f m={anchor_m:.7f} raw={comp['raw_L']:.15f} "
        f"status={sol['solver_status']} active_freq={comp['selected_frequency_count']} "
        f"solve_seconds={sol['solve_elapsed_seconds']:.2f}",
        flush=True,
    )
    budget_row = _row_for_exact_budget(comp, anchor_m)
    budget = e1c._exact_budget_for_row(budget_row, grid_n=ROOT_GRID_N, dps=EXACT_DPS)
    row = _hardened_row(comp, budget, anchor_m)
    payload = {
        "schema": "public cached exact-hardened SOC E4f row v1",
        "family": "e4f",
        "anchor_m": float(anchor_m),
        "comb_description": _comb_description(),
        "solver_status": str(sol["solver_status"]),
        "nominal_raw_L": float(comp["raw_L"]),
        "nominal_solver_objective_L": float(sol["solver_objective_L"]),
        "selected_frequency_count": int(comp["selected_frequency_count"]),
        "zero_pruned_count": int(comp["zero_pruned_count"]),
        "solve_elapsed_seconds": float(sol["solve_elapsed_seconds"]),
        "total_elapsed_seconds": float(time.time() - started),
        "exact_shift": float(budget["certified_shift_upper"]),
        "raw_budget_hi": float(budget["raw_budget_hi"]),
        "shifted_budget_hi": float(budget["shifted_budget_checks"][-1]["budget_hi"]),
        "positive_part_margin": float(budget["positive_part_margin"]),
        "root_count": int(budget["root_count"]),
        "root_width_max": float(budget["root_width_max"]),
        "lost_float_bracket_count": int(budget["lost_float_bracket_count"]),
        "unsafe_same_sign_cell_count": int(budget["unsafe_same_sign_cell_count"]),
        "real_generator_gate_pass": bool(row["real_generator_gate_pass"]),
        "beta_active_count_gt_1e_8": int(row["beta_active_count_gt_1e_8"]),
        "row_value_at_source_m": float(row["L_source_m_lower"]),
        "row": row,
        "cache_status": "newly_solved",
    }
    _write_json(_cache_path(anchor_m), payload)
    print(
        f"row_hardened_done family=e4f m={anchor_m:.7f} "
        f"L_source={payload['row_value_at_source_m']:.15f} "
        f"shift={payload['exact_shift']:.12e} budget_margin={payload['positive_part_margin']:.3e} "
        f"elapsed={payload['total_elapsed_seconds']:.1f}",
        flush=True,
    )
    return payload


def _row_value(row: dict[str, Any], m: float) -> float:
    mm = float(m)
    return float(row["quadratic_c0_lower"]) + float(row["a1"]) * mm + 0.5 * float(row["a2"]) * mm * mm


def _best_row_value(rows: list[dict[str, Any]], m: float) -> dict[str, Any]:
    if not rows:
        return {"status": "no_rows", "value": None, "row_id": None}
    vals = np.asarray([_row_value(row, m) for row in rows], dtype=np.float64)
    idx = int(np.argmax(vals))
    return {"status": "success", "value": float(vals[idx]), "row_id": rows[idx]["id"], "family": rows[idx].get("family")}


def _best_outside_passing(envelopes_by_bracket: dict[str, dict[str, Any]], group: str) -> dict[str, Any]:
    best: dict[str, Any] | None = None
    by_bracket: dict[str, Any] = {}
    for bracket_key, envs in envelopes_by_bracket.items():
        env = envs[group]
        if env["outside_exclusion_pass"]:
            entry = {
                "active_bracket": bracket_key,
                "group": group,
                "certified_global": float(env["certified_global"]),
                "global_argmin_m": float(env["global_argmin_m"]),
                "global_controlling_row": env["global_controlling_row"],
                "outside_exclusion_margin": float(env["outside_exclusion_margin"]),
            }
        else:
            entry = {
                "active_bracket": bracket_key,
                "group": group,
                "status": "outside_exclusion_failed",
                "candidate_global": float(env["certified_global"]),
                "outside_exclusion_margin": float(env["outside_exclusion_margin"]),
            }
        by_bracket[bracket_key] = entry
        if "certified_global" in entry and (best is None or entry["certified_global"] > best["certified_global"]):
            best = entry
    return {"by_bracket": by_bracket, "overall": best}


def _envelopes_by_bracket(
    base_rows: list[dict[str, Any]],
    inherited_rows: list[dict[str, Any]],
    prior_rows: list[dict[str, Any]],
    e4f_rows: list[dict[str, Any]],
    base_repro: dict[str, Any],
    mean_guard: float,
) -> dict[str, dict[str, Any]]:
    e2f_rows = [r for r in prior_rows if r.get("family") == "e2f"]
    e3f_rows = [r for r in prior_rows if r.get("family") == "e3f"]
    out: dict[str, dict[str, Any]] = {}
    for blo, bhi in ACTIVE_BRACKETS:
        key = f"{blo:.3f}_{bhi:.3f}"
        out[key] = {
            "e1_inherited": sfe._apply_baseline_floor(
                sfe._envelope(
                    list(base_rows) + inherited_rows,
                    mean_guard,
                    "inherited_E1_dense_rows",
                    active_bracket=(blo, bhi),
                ),
                base_repro,
            ),
            "e2f_density": sfe._apply_baseline_floor(
                sfe._envelope(
                    list(base_rows) + inherited_rows + e2f_rows,
                    mean_guard,
                    "inherited_plus_E2f_rows",
                    active_bracket=(blo, bhi),
                ),
                base_repro,
            ),
            "e4f_density": sfe._apply_baseline_floor(
                sfe._envelope(
                    list(base_rows) + inherited_rows + e2f_rows + e4f_rows,
                    mean_guard,
                    "inherited_plus_E2f_E4f_rows",
                    active_bracket=(blo, bhi),
                ),
                base_repro,
            ),
            "prior_union": sfe._apply_baseline_floor(
                sfe._envelope(
                    list(base_rows) + inherited_rows + e2f_rows + e3f_rows,
                    mean_guard,
                    "inherited_plus_prior_E2f_E3f_rows",
                    active_bracket=(blo, bhi),
                ),
                base_repro,
            ),
            "union_all": sfe._apply_baseline_floor(
                sfe._envelope(
                    list(base_rows) + inherited_rows + prior_rows + e4f_rows,
                    mean_guard,
                    "inherited_plus_E2f_E3f_E4f_rows",
                    active_bracket=(blo, bhi),
                ),
                base_repro,
            ),
        }
    return out


def _payload_summary(payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "family": str(p["family"]),
            "anchor_m": float(p["anchor_m"]),
            "nominal_raw_L": float(p["nominal_raw_L"]),
            "row_value_at_source_m": float(p["row_value_at_source_m"]),
            "selected_frequency_count": int(p["selected_frequency_count"]),
            "exact_shift": float(p["exact_shift"]),
            "positive_part_margin": float(p["positive_part_margin"]),
            "root_count": int(p["root_count"]),
            "cache_path": str(_cache_path(float(p["anchor_m"])) if p["family"] == "e4f" else p.get("cache_path", "")),
            "cache_status": str(p.get("cache_status", "unknown")),
        }
        for p in payloads
    ]


def _trend_summary(envelopes: dict[str, dict[str, Any]]) -> dict[str, Any]:
    e1 = _best_outside_passing(envelopes, "e1_inherited")
    e2f = _best_outside_passing(envelopes, "e2f_density")
    e4f = _best_outside_passing(envelopes, "e4f_density")
    points = {"E1_0p125": e1, "E2f_0p0625": e2f, "E4f_0p03125": e4f}
    vals = {
        name: (entry["overall"]["certified_global"] if entry["overall"] is not None else None)
        for name, entry in points.items()
    }
    inc_e1_e2f = None if vals["E1_0p125"] is None or vals["E2f_0p0625"] is None else vals["E2f_0p0625"] - vals["E1_0p125"]
    inc_e2f_e4f = None if vals["E2f_0p0625"] is None or vals["E4f_0p03125"] is None else vals["E4f_0p03125"] - vals["E2f_0p0625"]
    if inc_e1_e2f is None or inc_e2f_e4f is None:
        verdict = "incomplete"
    elif inc_e2f_e4f <= 0.5 * inc_e1_e2f:
        verdict = "shrinking_converging"
    elif inc_e2f_e4f >= 0.75 * inc_e1_e2f:
        verdict = "steady_still_rising"
    else:
        verdict = "moderate_shrink_inconclusive"
    return {
        "points": points,
        "certified_values": vals,
        "increment_E1_to_E2f": inc_e1_e2f,
        "increment_E2f_to_E4f": inc_e2f_e4f,
        "increment_ratio_E2f_E4f_over_E1_E2f": (
            None if inc_e1_e2f in (None, 0.0) or inc_e2f_e4f is None else inc_e2f_e4f / inc_e1_e2f
        ),
        "verdict": verdict,
        "verdict_rule": "shrinking if E2f->E4f <= 0.5*(E1->E2f); steady if >= 0.75*(E1->E2f)",
    }


def _boundary_summary(
    base_rows: list[dict[str, Any]],
    inherited_rows: list[dict[str, Any]],
    prior_rows: list[dict[str, Any]],
    e4f_rows: list[dict[str, Any]],
) -> dict[str, Any]:
    e2f_rows = [r for r in prior_rows if r.get("family") == "e2f"]
    prior_controller = _best_row_value(list(base_rows) + inherited_rows + e2f_rows, 0.0)
    e4f_controller = _best_row_value(e4f_rows, 0.0)
    combined_controller = _best_row_value(list(base_rows) + inherited_rows + e2f_rows + e4f_rows, 0.0)
    e4f_m0 = next((r for r in e4f_rows if abs(float(r["m"]) - 0.0) <= 5.0e-13), None)
    e4f_m001 = next((r for r in e4f_rows if abs(float(r["m"]) - 0.001) <= 5.0e-13), None)
    prior_value = prior_controller.get("value")
    combined_value = combined_controller.get("value")
    return {
        "prior_m0_controller": prior_controller,
        "best_e4f_at_m0": e4f_controller,
        "combined_m0_controller": combined_controller,
        "e4f_anchor_m0_value_at_m0": None if e4f_m0 is None else _row_value(e4f_m0, 0.0),
        "e4f_anchor_m001_value_at_m0": None if e4f_m001 is None else _row_value(e4f_m001, 0.0),
        "e4f_dominates_prior_m0_controller": bool(
            prior_value is not None and combined_value is not None and combined_value > prior_value
        ),
        "boundary_lift": None if prior_value is None or combined_value is None else combined_value - prior_value,
    }


def _assemble_result(
    *,
    started: float,
    base_rows: list[dict[str, Any]],
    inherited_payloads: list[dict[str, Any]],
    prior_payloads: list[dict[str, Any]],
    e4f_payloads: list[dict[str, Any]],
    skipped: list[dict[str, Any]],
    recovered: dict[str, Any],
    base_repro: dict[str, Any],
    inclusion_check: dict[str, Any],
    mean_guard: float,
    status: str,
) -> dict[str, Any]:
    inherited_rows = [p["row"] for p in inherited_payloads]
    prior_rows = [p["row"] for p in prior_payloads]
    e4f_rows = [p["row"] for p in e4f_payloads]
    envelopes = _envelopes_by_bracket(base_rows, inherited_rows, prior_rows, e4f_rows, base_repro, mean_guard)
    best_union = _best_outside_passing(envelopes, "union_all")
    trend = _trend_summary(envelopes)
    boundary = _boundary_summary(base_rows, inherited_rows, prior_rows, e4f_rows)
    union_003 = envelopes["0.000_0.030"]["union_all"]
    union_004 = envelopes["0.000_0.040"]["union_all"]
    e4f_003 = envelopes["0.000_0.030"]["e4f_density"]
    e4f_004 = envelopes["0.000_0.040"]["e4f_density"]
    all_e4f_rows_valid = bool(
        all(p["row"]["exact_positive_part_budget"]["exact_budget_pass"] for p in e4f_payloads)
        and all(p["row"]["real_generator_gate_pass"] for p in e4f_payloads)
    )
    return {
        "schema": "public SOC frequency density point E4f v1",
        "status": status,
        "elapsed_seconds": float(time.time() - started),
        "base_reproduction": base_repro,
        "base_reproduction_expected": BASE_CERTIFIED_GLOBAL,
        "recovered_exact_budget_row": recovered,
        "inclusion_check": inclusion_check,
        "e4f_anchors_requested": E4F_ANCHORS_REQUESTED,
        "e4f_priority_schedule": E4F_PRIORITY_SCHEDULE,
        "completed_e4f_row_count": int(len(e4f_payloads)),
        "completed_e4f_rows": _payload_summary(e4f_payloads),
        "skipped_or_failed_rows": skipped,
        "inherited_payload_count": int(len(inherited_payloads)),
        "prior_enrichment_payload_count": int(len(prior_payloads)),
        "prior_e2f_payload_count": int(sum(1 for p in prior_payloads if p["family"] == "e2f")),
        "prior_e3f_payload_count": int(sum(1 for p in prior_payloads if p["family"] == "e3f")),
        "bracket_envelopes": envelopes,
        "best_outside_passing_union_all": best_union,
        "comb_density_trend": trend,
        "m0_boundary_resolution": boundary,
        "gate_summary": {
            "base_reproduction": bool(base_repro["pass"]),
            "mean_guard_matches_request": bool(abs(float(mean_guard) - MEAN_GUARD_EXPECTED) <= 1.0e-15),
            "inclusion": bool(inclusion_check["all_requested_inclusions_pass"]),
            "e4f_exact_budget_all_completed": all_e4f_rows_valid,
            "e4f_all_requested_anchors_completed": bool(len(e4f_payloads) == len(E4F_ANCHORS_REQUESTED)),
            "union_0_03_outside_exclusion": bool(union_003["outside_exclusion_pass"]),
            "union_0_04_outside_exclusion": bool(union_004["outside_exclusion_pass"]),
            "e4f_density_0_03_outside_exclusion": bool(e4f_003["outside_exclusion_pass"]),
            "e4f_density_0_04_outside_exclusion": bool(e4f_004["outside_exclusion_pass"]),
            "union_smoke_under_known_feasible": bool(
                union_003["certified_global"] <= KNOWN_FEASIBLE_GUARD
                and union_004["certified_global"] <= KNOWN_FEASIBLE_GUARD
            ),
            "certified_claim_allowed": bool(
                base_repro["pass"]
                and inclusion_check["all_requested_inclusions_pass"]
                and all_e4f_rows_valid
                and e4f_003["outside_exclusion_pass"]
                and e4f_004["outside_exclusion_pass"]
            ),
        },
        "settings": {
            "solver": "CLARABEL",
            "solver_tol": 1.0e-8,
            "solver_max_iter": 500,
            "x_grid_points": 1001,
            "exact_budget_root_grid_n": ROOT_GRID_N,
            "root_tol": ROOT_TOL,
            "exact_dps": EXACT_DPS,
            "domain_partitions": DOMAIN_PARTITIONS,
            "refined_partitions": REFINED_PARTITIONS,
            "mean_guard": float(mean_guard),
            "known_feasible_guard": KNOWN_FEASIBLE_GUARD,
        },
        "reference_values": {
            "baseline_base": BASE_CERTIFIED_GLOBAL,
            "E1_0p125_prior": E1_CERTIFIED_VALUE,
            "E2f_0p0625_prior": E2F_CERTIFIED_VALUE,
        },
        "source_paths": {
            "base_certificate": str(BASE_CERT_PATH),
            "base_exact_budget": str(BASE_EXACT_BUDGET_PATH),
            "prior_enrichment_json": str(PRIOR_JSON),
            "output_json": str(OUT_JSON),
            "output_json_root_copy": str(OUT_JSON_ROOT_COPY),
            "row_cache_dir": str(ROW_CACHE_DIR),
        },
    }


def _print_summary(result: dict[str, Any]) -> None:
    print("soc_freq_density3_summary_begin", flush=True)
    br = result["base_reproduction"]
    print(
        f"base_reproduction_integrity={br['value']:.15f} "
        f"expected={BASE_CERTIFIED_GLOBAL:.15f} argmin_m={br['argmin_m']:.7f} pass={br['pass']}",
        flush=True,
    )
    inc = result["inclusion_check"]
    print(
        f"e4f_inclusion base={inc['base_frequency_count']} e2f={inc['e2f_frequency_count']} "
        f"e4f={inc['e4f_frequency_count']} strict_superset_e2f={inc['e4f_strict_superset_of_e2f']} "
        f"extra_vs_e2f={inc['extra_vs_e2f_count']}",
        flush=True,
    )
    for bracket_key, envs in result["bracket_envelopes"].items():
        for group in ("e1_inherited", "e2f_density", "e4f_density", "union_all"):
            env = envs[group]
            print(
                f"envelope bracket={bracket_key} group={group} value={env['certified_global']:.15f} "
                f"argmin={env['global_argmin_m']:.7f} controlling={env['global_controlling_row']} "
                f"outside_pass={env['outside_exclusion_pass']} margin={env['outside_exclusion_margin']:.12e}",
                flush=True,
            )
    trend = result["comb_density_trend"]
    vals = trend["certified_values"]
    print(
        f"density_trend E1={vals['E1_0p125'] if vals['E1_0p125'] is not None else 'None'} "
        f"E2f={vals['E2f_0p0625'] if vals['E2f_0p0625'] is not None else 'None'} "
        f"E4f={vals['E4f_0p03125'] if vals['E4f_0p03125'] is not None else 'None'} "
        f"inc_E1_E2f={trend['increment_E1_to_E2f']} "
        f"inc_E2f_E4f={trend['increment_E2f_to_E4f']} verdict={trend['verdict']}",
        flush=True,
    )
    boundary = result["m0_boundary_resolution"]
    print(
        f"m0_boundary prior={boundary['prior_m0_controller']} "
        f"best_e4f={boundary['best_e4f_at_m0']} "
        f"combined={boundary['combined_m0_controller']} "
        f"lift={boundary['boundary_lift']} dominates_prior={boundary['e4f_dominates_prior_m0_controller']}",
        flush=True,
    )
    best = result["comb_density_trend"]["points"]["E4f_0p03125"]["overall"]
    if best is not None:
        print(
            f"best_e4f_outside_passing value={best['certified_global']:.15f} "
            f"bracket={best['active_bracket']} argmin={best['global_argmin_m']:.7f} "
            f"controlling={best['global_controlling_row']} "
            f"delta_vs_baseline={best['certified_global'] - BASE_CERTIFIED_GLOBAL:.12e}",
            flush=True,
        )
    print(f"saved_json={OUT_JSON}", flush=True)
    print(f"saved_json_root_copy={OUT_JSON_ROOT_COPY}", flush=True)


def run_frequency_density3_analysis(*, max_total_seconds: float = 1680.0) -> dict[str, Any]:
    started = time.time()
    base_cert = _load_json(BASE_CERT_PATH)
    exact_budget_summary = _load_json(BASE_EXACT_BUDGET_PATH)
    mean_guard = float(base_cert["verification_settings"]["mean_guard"])
    base_rows, recovered = smh.build_verified_rows_with_baseline_recovery(base_cert, exact_budget_summary)
    base_repro = sfe._base_reproduction(base_rows, mean_guard)
    if not base_repro["pass"]:
        raise RuntimeError(f"base reproduction integrity control failed: {base_repro}")

    freqs = _build_frequency_sets(e1c._base_frequencies(base_cert))
    inclusion_check = _inclusion_check(freqs)
    if not inclusion_check["all_requested_inclusions_pass"]:
        raise RuntimeError(f"E4f inclusion check failed: {inclusion_check}")

    print(
        f"base_reproduction_integrity={base_repro['value']:.15f} "
        f"expected={BASE_CERTIFIED_GLOBAL:.15f} argmin_m={base_repro['argmin_m']:.7f}",
        flush=True,
    )
    print(
        f"inclusion_check base={inclusion_check['base_frequency_count']} "
        f"e2f={inclusion_check['e2f_frequency_count']} e4f={inclusion_check['e4f_frequency_count']} "
        f"e4f_superset_e2f={inclusion_check['e4f_strict_superset_of_e2f']}",
        flush=True,
    )

    inherited_payloads = fe2._load_inherited_payloads()
    prior_payloads = _load_prior_enrichment_payloads()
    print(
        f"inherited_hardened_rows_loaded={len(inherited_payloads)} "
        f"prior_enrichment_rows_loaded={len(prior_payloads)}",
        flush=True,
    )

    completed: list[dict[str, Any]] = []
    skipped: list[dict[str, Any]] = []
    for anchor_m in E4F_PRIORITY_SCHEDULE:
        elapsed = time.time() - started
        cached = _load_cached_e4f_row(anchor_m)
        if cached is None and elapsed > float(max_total_seconds) - 240.0:
            skipped.append(
                {
                    "family": "e4f",
                    "anchor_m": float(anchor_m),
                    "reason": "compute_limit_guard",
                    "elapsed_seconds_before_row": float(elapsed),
                }
            )
            print(f"row_skipped_compute_guard family=e4f m={anchor_m:.7f} elapsed={elapsed:.1f}", flush=True)
            continue
        try:
            payload = _solve_and_harden_e4f_row(freqs["e4f"], anchor_m)
        except Exception as exc:  # noqa: BLE001
            skipped.append({"family": "e4f", "anchor_m": float(anchor_m), "reason": "row_exception", "error": repr(exc)})
            print(f"row_failed family=e4f m={anchor_m:.7f} error={exc!r}", flush=True)
            continue
        completed.append(payload)
        partial = _assemble_result(
            started=started,
            base_rows=base_rows,
            inherited_payloads=inherited_payloads,
            prior_payloads=prior_payloads,
            e4f_payloads=completed,
            skipped=skipped,
            recovered=recovered,
            base_repro=base_repro,
            inclusion_check=inclusion_check,
            mean_guard=mean_guard,
            status="partial_running",
        )
        _write_json(OUT_JSON, partial)
        _write_json(OUT_JSON_ROOT_COPY, partial)

    status = "complete_within_time_guard" if not skipped else "compute_limited"
    result = _assemble_result(
        started=started,
        base_rows=base_rows,
        inherited_payloads=inherited_payloads,
        prior_payloads=prior_payloads,
        e4f_payloads=completed,
        skipped=skipped,
        recovered=recovered,
        base_repro=base_repro,
        inclusion_check=inclusion_check,
        mean_guard=mean_guard,
        status=status,
    )
    _write_json(OUT_JSON, result)
    _write_json(OUT_JSON_ROOT_COPY, result)
    _print_summary(result)
    return result


if __name__ == "__main__":
    run_frequency_density3_analysis()
